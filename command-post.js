...
class GuerrillaCommandPost {
    constructor() {
        ...
        this.getState = function(key, fallback) {
            if (window.getState) {
        ...
        this.updateState = function(key, updater) {
    ...
    setupLoginListeners() {
        ...
        if (loginForm) {
            ...
            console.log('Login form listener attached successfully');
        } else {
            ...
            recurring: [
                {
                    ...
                    status: "active",
                    serviceType: "seo", // seo | google_ads | meta_ads
                    packageTier: "silver", // bronze | silver | gold
                    optimizedMonths: ["2024-02"],
                    tasks: [
            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                    ...
                    <div class="flex items-center space-x-4 text-sm">
                        ...
                        ${type === 'recurring' ? `<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-brand-layer3 text-gray-300">${(job.serviceType||'').replace('_',' ') || 'service'}</span>` : ''}
                        ${type === 'recurring' && job.packageTier ? `<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-brand-layer3 text-gray-300">${job.packageTier}</span>` : ''}
                        ${type === 'recurring' && this.isJobOptimizedThisMonth(job) ? `<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-600 text-white">Optimized</span>` : ''}
                    </div>
                </div>
    ...
    openJobDetail(job) {
        // Dedicated Job Detail modal (separate from Client Detail)
        const existing = document.getElementById('jobDetailModal');
        if (existing) existing.remove();

        const isRecurring = !!job.nextDue || job.frequency === 'monthly';
        ...
        modal.innerHTML = `
            <div class="bg-brand-layer1 rounded-2xl shadow-2xl border border-brand-border max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                <div class="p-6">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-2xl font-bold text-white">Job — ${job.title}</h2>
                        <button class="text-gray-400 hover:text-white transition-colors" data-close-modal>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div class="bg-brand-layer2 border border-brand-border rounded-lg p-4">
                            ...
                            <select id="jobServiceType" class="w-full px-3 py-2 bg-brand-layer3 border border-brand-border rounded text-white">
                                <option value="seo" ${job.serviceType==='seo'?'selected':''}>SEO</option>
                                <option value="google_ads" ${job.serviceType==='google_ads'?'selected':''}>Google Ads</option>
                                <option value="meta_ads" ${job.serviceType==='meta_ads'?'selected':''}>Meta Ads</option>
                            </select>
                        </div>
                        <div class="bg-brand-layer2 border border-brand-border rounded-lg p-4">
                            <div class="text-xs text-gray-400 mb-1">Package</div>
                            <select id="jobPackageTier" class="w-full px-3 py-2 bg-brand-layer3 border border-brand-border rounded text-white">
                                <option value="bronze" ${job.packageTier==='bronze'?'selected':''}>Bronze</option>
                                <option value="silver" ${job.packageTier==='silver'?'selected':''}>Silver</option>
                                <option value="gold" ${job.packageTier==='gold'?'selected':''}>Gold</option>
                            </select>
                        </div>` : ''}
                    </div>
                    <div class="bg-brand-layer2 border border-brand-border rounded-lg p-4 mb-4">
                        <div class="text-xs text-gray-400 mb-1">Description</div>
                        <div class="text-gray-300">${job.description || ''}</div>
                    </div>
                    ${isRecurring ? `
                    <div class="bg-brand-layer2 border border-brand-border rounded-lg p-4 mb-4 flex items-center justify-between">
                        <div class="text-white">Optimized for ${this.getCurrentMonthKey()}</div>
                        <label class="inline-flex items-center gap-2">
                            <input id="optimizedToggle" type="checkbox" ${this.isJobOptimizedThisMonth(job) ? 'checked' : ''} />
                            <span class="text-sm text-gray-300">Mark Optimized</span>
                        </label>
                    </div>` : ''}
                    <div class="bg-brand-layer2 border border-brand-border rounded-lg p-4">
        ...
        // Save handler (service/package/optimized)
        document.getElementById('saveJobDetailBtn')?.addEventListener('click', () => {
            if (isRecurring) {
                const serviceSel = document.getElementById('jobServiceType');
                const pkgSel = document.getElementById('jobPackageTier');
                const optimized = document.getElementById('optimizedToggle');
                if (serviceSel) job.serviceType = serviceSel.value;
                if (pkgSel) job.packageTier = pkgSel.value;
                if (optimized) this.setJobOptimizedForCurrentMonth(job, optimized.checked);
                this.saveJobsData();
                // Refresh list
                this.renderJobs('recurring');
            }
    ...
    isJobOptimizedThisMonth(job) {
        const key = this.getCurrentMonthKey();
        const list = Array.isArray(job.optimizedMonths) ? job.optimizedMonths : [];
        return list.includes(key);
    }

    setJobOptimizedForCurrentMonth(job, optimized) {
        const key = this.getCurrentMonthKey();
        if (!Array.isArray(job.optimizedMonths)) job.optimizedMonths = [];
        const has = job.optimizedMonths.includes(key);
        if (optimized && !has) job.optimizedMonths.push(key);
        if (!optimized && has) job.optimizedMonths = job.optimizedMonths.filter(k => k !== key);
    }
    ...
    // Drawer for viewing a single digital asset in detail
    openDigitalAssetDrawer(assetId) {
        if (!this.currentClientDetail) return;
        const clientId = this.currentClientDetail.id;
        const assets = this.digitalAssetsData[clientId] || [];
        const asset = assets.find(a => String(a.id) === String(assetId));
        if (!asset) return;

        // Remove any existing drawer
        const existing = document.getElementById('digitalAssetDrawer');
        if (existing) existing.remove();

        const overlay = document.createElement('div');
        overlay.id = 'digitalAssetDrawer';
        overlay.className = 'fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm z-50 flex justify-end';
        overlay.innerHTML = `
            <div class="w-full max-w-xl h-full bg-brand-layer1 border-l border-brand-border flex flex-col">
                <div class="p-4 border-b border-brand-border flex items-center justify-between">
                    <h3 class="text-white font-semibold">Digital Asset</h3>
                    <button class="text-gray-400 hover:text-white" id="closeDigitalAssetDrawerBtn">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </button>
                ...
                <div class="p-4 space-y-3 overflow-auto">
                    ...
                    <div class="text-white">${asset.type || ''}</div>

                    <div class="text-sm text-gray-400">Name</div>
                    <div class="text-white">${asset.name || '—'}</div>

                    <div class="text-sm text-gray-400">Ownership</div>
                    <div class="text-white">${asset.ownership || '—'}</div>

                    <div class="text-sm text-gray-400">Access Status</div>
                    <div class="text-white">${asset.accessStatus || '—'}</div>

                    <div class="text-sm text-gray-400">External ID</div>
                    <div class="text-white">${asset.externalId || '—'}</div>

                    <div class="text-sm text-gray-400">Account URL</div>
                    <div class="break-all"><a class="text-brand-primary hover:text-brand-secondary" href="${asset.accountUrl || '#'}" target="_blank">${asset.accountUrl || '—'}</a></div>
        ...
        overlay.querySelector('#closeDigitalAssetDrawerBtn')?.addEventListener('click', close);
        overlay.querySelector('#closeDigitalAssetDrawerBtn2')?.addEventListener('click', close);
    }
    ...
    closeModal(modalSelector = '.fixed') {
        // Backward-compatible remover for dynamic overlays
        const modal = document.querySelector(modalSelector);
        if (modal) {
            // If this looks like a static modal (has an id that ends with Modal), hide instead of removing
            const isStatic = modal.id && /Modal$/i.test(modal.id);
            if (isStatic) {
                modal.classList.add('hidden');
            } else {
                modal.remove();
        ...
        // Fallback: hide any visible static modal
        const visibleStatic = Array.from(document.querySelectorAll('[id$="Modal"]')).find(m => !m.classList.contains('hidden'));
        if (visibleStatic) {
            visibleStatic.classList.add('hidden');
            return;
        }
        // Final fallback: remove the top-most dynamic overlay
        const topOverlay = Array.from(document.querySelectorAll('.fixed')).reverse().find(el => getComputedStyle(el).display !== 'none');
        if (topOverlay) topOverlay.remove();
    }
    
    // Setup modal close handlers
    setupModalCloseHandlers() {
        // Delegated close for X, Cancel, explicit close/cancel ids, or .close-modal
        document.addEventListener('click', (e) => {
            const target = e.target;
            const text = (target.textContent || '').trim().toLowerCase();
            const byClass = target.classList.contains('close-modal') || target.closest('.close-modal');
            const byText = text === 'cancel' || text === 'x' || text === 'close';
            const byId = (target.id || '').startsWith('close') || (target.id || '').startsWith('cancel');
            const byData = target.closest('[data-close-modal]');
            if (byClass || byText || byId || byData) {
                e.preventDefault();
                const container = target.closest('[id$="Modal"]') || target.closest('.fixed');
                if (container) {
                    if (container.id && /Modal$/i.test(container.id)) {
    ...
    closeAddMeetingTranscriptionModal() {
        ...
        const form = document.getElementById('addMeetingTranscriptionForm');
        if (form) form.reset();
    }
    ...
    loadProfileSettings() {
        ...
        console.log('this.getState method:', typeof this.getState);
        console.log('window.getState method:', typeof window.getState);
        try {
            ...
            if (profileRoleEl) profileRoleEl.value = profile.role || '';
            ...
            if (profileLocationEl) profileLocationEl.value = profile.location || '';
            const profileNotesEl = document.getElementById('profileNotes');
            if (profileNotesEl) profileNotesEl.value = profile.notes || '';
    ...
    loadNotificationSettings() {
        ...
        try {
            ...
            const taskAddedEl = document.getElementById('taskAdded');
            if (taskAddedEl) taskAddedEl.checked = notifications.taskAdded || false;
            const taskOverdueEl = document.getElementById('taskOverdue');
            if (taskOverdueEl) taskOverdueEl.checked = notifications.taskOverdue || false;
            const dailySummaryEl = document.getElementById('dailySummary');
            if (dailySummaryEl) dailySummaryEl.checked = notifications.dailySummary !== false;
    ...
    loadIntegrationSettings() {
        ...
        try {
            ...
            if (gmailEmailEl) gmailEmailEl.value = integrations.gmail?.email || '';
            
            // Load webhook settings
            const webhooks = integrations.webhooks || {};
            const inboundWebhookUrlEl = document.getElementById('inboundWebhookUrl');
            if (inboundWebhookUrlEl) inboundWebhookUrlEl.value = webhooks.inboundUrl || '';
            const referencePayloadEl = document.getElementById('referencePayload');
            if (referencePayloadEl) referencePayloadEl.value = webhooks.referencePayload ? JSON.stringify(webhooks.referencePayload, null, 2) : '';
            ...
            const triggerTaskCompletedEl = document.getElementById('triggerTaskCompleted');
            if (triggerTaskCompletedEl) triggerTaskCompletedEl.checked = triggers.taskCompleted || false;
            const triggerTaskInProgressEl = document.getElementById('triggerTaskInProgress');
            if (triggerTaskInProgressEl) triggerTaskInProgressEl.checked = triggers.taskInProgress || false;
            const triggerTaskOverdueEl = document.getElementById('triggerTaskOverdue');
            if (triggerTaskOverdueEl) triggerTaskOverdueEl.checked = triggers.taskOverdue || false;
            const triggerJobInProgressEl = document.getElementById('triggerJobInProgress');
            if (triggerJobInProgressEl) triggerJobInProgressEl.checked = triggers.jobInProgress || false;
            const triggerJobNeedsAssistanceEl = document.getElementById('triggerJobNeedsAssistance');
            if (triggerJobNeedsAssistanceEl) triggerJobNeedsAssistanceEl.checked = triggers.jobNeedsAssistance || false;
...
document.addEventListener('DOMContentLoaded', () => {
    window.guerrillaCommandPost = new GuerrillaCommandPost();
});
